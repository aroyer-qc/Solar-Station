...
static err_t _tcp_new_api(struct tcpip_api_call_data *api_call_msg){
    ...
    msg->pcb = tcp_new_ip_type((u8_t)msg->ipType);
    msg->err = msg->pcb ? ERR_OK : ERR_MEM;
    return msg->err;
}

static tcp_pcb * _tcp_new_ip_type_api(uint8_t ipType) {
    tcp_api_call_t msg;
    msg.pcb = NULL;
    msg.closed_slot = -1;
    msg.ipType = ipType;
...
static err_t _tcp_set_callbacks_api(struct tcpip_api_call_data *api_call_msg){
    ...
    tcp_arg(msg->pcb, msg->callbacks.arg);

    // lwIP only allows recv/sent/poll callbacks on established/non-LISTEN pcbs.
    if(msg->pcb->state != LISTEN){
        tcp_recv(msg->pcb, msg->callbacks.recv);
        tcp_sent(msg->pcb, msg->callbacks.sent);
        tcp_err(msg->pcb, msg->callbacks.errfn);

        if(msg->callbacks.setPoll){
            tcp_poll(msg->pcb, msg->callbacks.poll, msg->callbacks.pollInterval);
        }
    }

    if(msg->callbacks.setAccept){
        tcp_accept(msg->pcb, msg->callbacks.accept);
    }
...
static err_t _tcp_set_callbacks(tcp_pcb * pcb,
                                void* arg,
                                tcp_recv_fn recv,
                                tcp_sent_fn sent,
                                tcp_err_fn errfn,
    ...
    msg.pcb = pcb;
    msg.closed_slot = -1;
    msg.callbacks.arg = arg;
    msg.callbacks.recv = recv;
    msg.callbacks.sent = sent;
    msg.callbacks.errfn = errfn;
    msg.callbacks.poll = poll;
    msg.callbacks.pollInterval = pollInterval;
    msg.callbacks.accept = accept;
    msg.callbacks.setPoll = setPoll ? 1 : 0;
    msg.callbacks.setAccept = setAccept ? 1 : 0;
...
, next(NULL)
{
    ...
    if(_pcb){
        ...
        _tcp_set_callbacks(_pcb, this, &_tcp_recv, &_tcp_sent, &_tcp_error, &_tcp_poll, 1, NULL, true, false);
    }
}
...
bool AsyncClient::connect(IPAddress ip, uint16_t port){
    ...
    tcp_pcb* pcb = _tcp_new_ip_type_api(IPADDR_TYPE_V4);
    if (!pcb){
        log_e("pcb == NULL");
        return false;
    }

    _tcp_set_callbacks(pcb, this, &_tcp_recv, &_tcp_sent, &_tcp_error, &_tcp_poll, 1, NULL, true, false);
...
int8_t AsyncClient::_close(){
    ...
    if(_pcb) {
        //log_i("");
        _tcp_set_callbacks(_pcb, NULL, NULL, NULL, NULL, NULL, 0, NULL, true, false);
...
void AsyncClient::_error(int8_t err) {
    if(_pcb){
        _tcp_set_callbacks(_pcb, NULL, NULL, NULL, NULL, NULL, 0, NULL, false, false);
...
void AsyncServer::begin(){
    ...
    int8_t err;
    _pcb = _tcp_new_ip_type_api(IPADDR_TYPE_V4);
    if (!_pcb){
        log_e("_pcb == NULL");
        return;
    }
    ...
    _tcp_set_callbacks(_pcb, (void*)this, NULL, NULL, NULL, NULL, 0, &_s_accept, false, true);
}

void AsyncServer::end(){
    if(_pcb){
        _tcp_set_callbacks(_pcb, NULL, NULL, NULL, NULL, NULL, 0, NULL, false, true);
